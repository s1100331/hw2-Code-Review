...
class Role:#定義一個類別Role作為Warrior、Magician的父類別
    #將Warrior、Magician類別的共同方法__init__、draw()、damage()定義在新命的Role類別中
    ...
    def method_attack(self):#新創一個method_attack()將子類別中的sword_attack()、magic_attack()合併、簡化，並放於父類別Role當中
        dir = 1
        if self.x >= 400:
            dir = -1
...
class Warrior(Role):#透過括號的方式來繼承父類別(Role)裡的方法
    def __init__(self, name, life, x, y, imgfile, tagname):
        super().__init__(name, life, x, y, imgfile, tagname)#使用super()呼叫父類別的initializer
        self.classname = 'Warrior'
        self.attack_mode = 'sword attack'

class Magician(Role):#透過括號的方式來繼承父類別(Role)裡的方法
    def __init__(self, name, life, x, y, imgfile, tagname):
        super().__init__(name, life, x, y, imgfile, tagname)#使用super()呼叫父類別的initializer
